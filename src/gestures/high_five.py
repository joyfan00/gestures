...
class HighFive(object):
    ...
    def move_to_high_five(self, high_five_angles=None):
        ...
        high_five_angles = {
            'left_w0': 2,
            'left_w1': 1,
            'left_w2': 0.5,         # Adjust the value for the left_w2 joint to lower the elbow
            'left_e0': 0,        # Adjust the value for the left_e0 joint to bend the elbow
            'left_e1': 0,
            'left_s0': 0,
            'left_s1': 0
        }
...
def main():
    print("HELLO")
    rospy.init_node("high_five")
    # rospy.wait_for_message("/robot/sim/started", Empty)
    limb = 'right'
    hover_distance = 0.15
    robot = HighFive(limb, hover_distance)
